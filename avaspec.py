...
# choose the DLL name you shipped alongside your .py
dll_name = "avaspecx64.dll"  # or avaspec.dll, as appropriate

# build full path and load
_lib_path = os.path.join(_here, dll_name)
...
def errcheck(retval, function, arguments):
    if retval < 0:
        raise RuntimeError("Error {0} occured during {1}".format(retval, inspect.stack()[1][3]))
    return arguments
...
def AVS_GetVersionInfo(handle, FPGAversion, FWversion, DLLversion):
    ...
    AVS_GetVersionInfo.errcheck = errcheck
...
def AVS_PrepareMeasure(handle, measconf):
    ...
    AVS_PrepareMeasure.errcheck = errcheck
